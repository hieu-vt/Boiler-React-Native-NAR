...
- ### Splash
  ...
  - Create new script in `package.json` file. ex:
    - `"splash-demo": "npx ts-node ./scripts/splash.ts splash/splash-demo.png 0E1019 150 demo BootSplashDemo",`
      ...
      - Open `new_boiler_arc.xcworkspace` via xcode then add `BootSplashDemo.storyboad` file to xcode.
...
- ### IOS
  ...
  - Open Open `new_boiler_arc.xcworkspace` via xcode
  - Select `new_boiler_arc-Dev` scheme then click edit scheme
  - Click Duplicate scheme.
  - Rename Scheme like: `new_boiler_arc-<SCHEME_SUFFIX>`. `SCHEME_SUFFIX` in env file
